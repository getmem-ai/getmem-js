...
export class APIError extends GetMemError {
    constructor(message, errorCode, details, requestId, statusCode) {
        super(message);
...
export class QuotaExceededError extends APIError {
    get current() { return this.details.current ?? 0; }
    get limit() { return this.details.limit ?? 0; }
    get resetAt() { return this.details.reset_at ?? ''; }
    constructor(message, details, requestId) {
...
export class ForbiddenError extends APIError {
    get requiredScope() { return this.details.required_scope ?? ''; }
    get availableScopes() { return this.details.available_scopes ?? []; }
    constructor(message, details, requestId) {
...
export class ValidationError extends APIError {
    get fieldErrors() { return this.details.field_errors ?? []; }
    constructor(message, details, requestId) {
...
export class RateLimitedError extends APIError {
    get retryAfter() { return this.details.retry_after ?? 60; }
    get current() { return this.details.current ?? 0; }
    get limit() { return this.details.limit ?? 0; }
    constructor(message, details, requestId) {
...
export function raiseForResponse(status, body) {
    ...
    if (Cls) {
        throw new Cls(body.message ?? `HTTP ${status}`, body.details ?? {}, body.request_id ?? '');
    }
    throw new APIError(body.message ?? `HTTP ${status}`, body.error ?? '', body.details ?? {}, body.request_id ?? '', status);
}
